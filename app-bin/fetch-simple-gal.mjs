...
function softExit(msg) {
  ...
  console.error('[fetch-simple-gal] Proceeding without a bundled binary (dev fallback will apply).')
  process.exit(0)
}
...
if (existsSync(targetBin)) {
  ...
  console.log(`[fetch-simple-gal] existing binary at ${targetBin} (${current ?? '?'}) — refreshing`)
}
